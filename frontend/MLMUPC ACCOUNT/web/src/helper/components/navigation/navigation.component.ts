...
import {
    ...
    HelperNavigationAppearance,
    HelperNavigationMode,
    HelperNavigationPosition,
} from 'helper/components/navigation/navigation.types';
...
import { HelperNavigationGroupItemComponent } from 'helper/components/navigation/group/group.component';
import { HelperScrollbarDirective } from 'helper/directives/scrollbar/scrollbar.directive';
...
export class HelperNavigationComponent
    implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
    ...
    @Input() appearance: HelperNavigationAppearance = 'default';
    @Input() autoCollapse: boolean = true;
    @Input() inner: boolean = false;
    @Input() mode: HelperNavigationMode = 'side';
    @Input() name: string = this._helperUtilsService.randomId();
    @Input() navigations: HelperNavigationItem[];
    @Input() opened: boolean = true;
    @Input() position: HelperNavigationPosition = 'left';
    @Input() transparentOverlay: boolean = false;
    @Output()
    readonly appearanceChanged: EventEmitter<HelperNavigationAppearance> =
        new EventEmitter<HelperNavigationAppearance>();
    @Output() readonly modeChanged: EventEmitter<HelperNavigationMode> =
        new EventEmitter<HelperNavigationMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> =
        new EventEmitter<boolean>();
    @Output()
    readonly positionChanged: EventEmitter<HelperNavigationPosition> =
        new EventEmitter<HelperNavigationPosition>();
    @ViewChild('navigationContent') private _navigationContentEl: ElementRef;

    activeAsideItemId: string | null = null;
    onCollapsableItemCollapsed: ReplaySubject<HelperNavigationItem> =
        new ReplaySubject<HelperNavigationItem>(1);
    onCollapsableItemExpanded: ReplaySubject<HelperNavigationItem> =
        new ReplaySubject<HelperNavigationItem>(1);
    ...
    private _helperScrollbarDirectives!: QueryList<HelperScrollbarDirective>;
    private _helperScrollbarDirectivesSubscription: Subscription;
    ...
    @HostBinding('class') get classList(): any {
        ...
        return {
            ...
            'helper-navigation-mode-over': this.mode === 'over',
    ...
    /**
     * Setter for helperScrollbarDirectives
     */
    @ViewChildren(HelperScrollbarDirective)
    set helperScrollbarDirectives(
        helperScrollbarDirectives: QueryList<HelperScrollbarDirective>
    ) {
        // Store the directives
        this._helperScrollbarDirectives = helperScrollbarDirectives;

        // Return if there are no directives
        if (helperScrollbarDirectives.length === 0) {
            return;
        }

        // Unsubscribe the previous subscriptions
        if (this._helperScrollbarDirectivesSubscription) {
            this._helperScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._helperScrollbarDirectivesSubscription = merge(
    ...
    ngOnInit(): void {
        ...
        if (this.name === '') {
            this.name = this._helperUtilsService.randomId();
        }

        // Register the navigation component
        this._helperNavigationService.registerComponent(this.name, this);
    ...
    ngAfterViewInit(): void {
        ...
        setTimeout(() => {
            ...
            // Otherwise
                ...
                this._helperScrollbarDirectives.forEach(
                    (helperScrollbarDirective) => {
                        // Skip if not enabled
                        if (!helperScrollbarDirective.isEnabled()) {
                            return;
                        }

                        // Scroll to the active element
                        helperScrollbarDirective.scrollToElement(
                            '.helper-navigation-item-active',
                            -120,
                            true
                        );
    ...
    ngOnDestroy(): void {
        ...
        this._helperNavigationService.deregisterComponent(this.name);
     ...
     * @param item
     */
    openAside(item: HelperNavigationItem): void {
     ...
     * @param item
     */
    toggleAside(item: HelperNavigationItem): void {
        // Toggle
    ...
    private _showOverlay(): void {
        ...
        this._overlay.classList.add('helper-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if (this.transparentOverlay) {
            this._overlay.classList.add(
                'helper-navigation-overlay-transparent'
            );
        }
